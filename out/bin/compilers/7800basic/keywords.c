...
void keywords(char **cstatement)
{
	...
	while (1) {
		...
		if (!strncmp(statement[0],"return\0",7)) prerror("return used as label");
		else if (statement[1][0]=='\0') {free(deallocstatement); freemem(deallocorstatement); freemem(deallocelstatement); return; }
		else if (statement[1][0]==' ') {free(deallocstatement); freemem(deallocorstatement); freemem(deallocelstatement); return; }
		...
		else if (!strncmp(statement[1],"drawhiscores\0",13)) drawhiscores(statement);
		else if (!strncmp(statement[1],"loadmemory\0",11)) loadmemory(statement);
		else if (!strncmp(statement[1],"savememory\0",11)) savememory(statement);
		else if (!strncmp(statement[1],"loadrombank\0",11)) loadrombank(statement);
		else if (!strncmp(statement[1],"loadrambank\0",11)) loadrambank(statement);
		else if (!strncmp(statement[1],"sdata\0",6)) sdata(statement);
		else if (!strncmp(statement[1],"speechdata\0",11)) speechdata(statement);
		else if (!strncmp(statement[1],"songdata\0",9)) songdata(statement);
		else if (!strncmp(statement[1],"stopsong\0",8)) stopsong();
		else if (!strncmp(statement[1],"playsong\0",9)) playsong(statement);
		else if (!strncmp(statement[1],"speak\0",6)) speak(statement);
		...
		else if (!strncmp(statement[1],"adjustvisible\0",14)) adjustvisible(statement);
		else if (!strncmp(statement[1],"lockzone\0",9)) lockzone(statement);
		else if (!strncmp(statement[1],"unlockzone\0",11)) unlockzone(statement);
		else if (!strncmp(statement[1],"gosub\0",6)) gosub(statement);
		else if (!strncmp(statement[1],"drawscreen\0",10)) drawscreen();
		else if (!strncmp(statement[1],"drawwait\0",8)) drawwait();
		else if (!strncmp(statement[1],"doublebuffer\0",11)) doublebuffer(statement);
		...
		else if (!strncmp(statement[1],"mutesfx\0",8)) mutesfx(statement);
		else if (!strncmp(statement[1],"memcpy\0",6)) domemcpy(statement);
		else if (!strncmp(statement[1],"memset\0",6)) domemset(statement);
		else if (!strncmp(statement[1],"asm\0",4)) doasm();
		else if (!strncmp(statement[1],"pop\0",4)) dopop();
		else if (!strncmp(statement[1],"rem\0",4)) {rem(statement); free(deallocstatement); freemem(deallocorstatement); freemem(deallocelstatement); return; }
		else if (!strncmp(statement[1],"asm\n",4)) doasm();
		else if (!strncmp(statement[1],"pop\n",4)) dopop();
		else if (!strncmp(statement[1],"rem\n",4)) {rem(statement); free(deallocstatement); freemem(deallocorstatement); freemem(deallocelstatement); return; }
		...
		else if (!strncmp(statement[1],"incbanner\0",9)) add_graphic(statement,1);
		else if (!strncmp(statement[1],"incmapfile\0",10)) incmapfile(statement);
		else if (!strncmp(statement[1],"newblock\0",8)) newblock();
		else if (!strncmp(statement[1],"voice\0",5)) voice(statement);
		else if (!strncmp(statement[1],"plotbanner\0",10)) plotbanner(statement);
		...
		else if (!strncmp(statement[1],"plotmapfile\0",11)) plotmapfile(statement);
	...
	free(deallocstatement); freemem(deallocorstatement); freemem(deallocelstatement);
}
