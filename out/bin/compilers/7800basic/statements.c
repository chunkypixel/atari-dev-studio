...
void plotmapfile(char **statement)
{
	...
	removeCR(statement[7]); height=strictatoi(statement[7]);
...
void incgraphic(char *file_name, int offset)
{
	...
	if(!png_get_tRNS(png_ptr, info_ptr, &trans, &num_trans,  NULL))
		...
		for(;t<(1<<bit_depth);t++)
		{
			sprintf(redefined_variables[numredefvars++],"%s_color%d = 0",graphicslabels[dmaplain][graphicsdatawidth[dmaplain]],t);
			sprintf(constants[numconstants++],"%s_color%d",graphicslabels[dmaplain][graphicsdatawidth[dmaplain]],t); // record to queue
		}
	}
	...
	if((graphicsdatawidth[dmaplain]>0)||(dmaplain>0)) //only process if the incgraphic command was encountered.
	{
		...
		for(currentplain=0;currentplain<=dmaplain;currentplain++)
		{
			...
			else 
			{
				ADDRBASE=BANKSTART-((dmaplain-currentplain)*DMASIZE);
				if(romat4k==1)
					ABADDRBASE=REALSTART+((currentbank-1)*0x4000)+0x2000-((dmaplain-currentplain)*DMASIZE); 
				else
			...
			if ( (currentplain<dmaplain) || ((currentplain==dmaplain) && (bankcount>0) && (currentbank+1<bankcount)) )
			{
				...
				else 
				{
					ADDRBASE=BANKSTART-((dmaplain-currentplain)*DMASIZE)+(DMASIZE/2);
					if(romat4k==1)
						ABADDRBASE=REALSTART+((currentbank-1)*0x4000)+0x2000-((dmaplain-currentplain)*DMASIZE)+(DMASIZE/2); 
					else
...
int findlabel(char **statement, int i)
{
	...
	if (!strncmp(statementcache,"incmapfile\0",10)) return 1;
	if (!strncmp(statementcache,"drawscreen\0",10)) return 1;
	if (!strncmp(statementcache,"drawwait\0",8)) return 1;
	...
	if (!strncmp(statementcache,"speak\0",6)) return 1;
	...
	if (!strncmp(statementcache,"plotmapfile\0",11)) return 1;
	...
	if (!strncmp(statementcache,"loadmemory\0",10)) return 1;
	if (!strncmp(statementcache,"savememory\0",10)) return 1;
	if (!strncmp(statementcache,"drawhiscroes\0",13)) return 1;
	if (!strncmp(statementcache,"memcpy\0",6)) return 1;
	if (!strncmp(statementcache,"memset\0",6)) return 1;
	...
	if (!strncmp(statementcache,"playsfx\0",8)) return 1;
	if (!strncmp(statementcache,"adjustvisible\0",14)) return 1;
	if (!strncmp(statementcache,"plotvalue\0",10)) return 1;
	if (!strncmp(statementcache,"newblock\0",9)) return 1;
	if (!strncmp(statementcache,"let\0",4)) return 1;
	if (!strncmp(statementcache,"dec\0",4)) return 1;
...
void songdata(char **statement)
{
	...
	if (!(optimization&4)) printf("	JMP .skip%d\n",templabel);
	...
	if (!(optimization&4)) printf(".skip%d\n",templabel);
	templabel=templabel+1;
}
...
{
	int tempovalue,paltempovalue,t;
	...
	while (1) 
	{
		...
		else if ((strncmp(data+1,"dictionary",10)==0)||(strncmp(data+1,"phonetic",8)==0))
		{
			...
			if(strncmp(data+1,"dictionary",10)==0)
				for(i=11;((data[i]!=0)&&(data[i]!='\r')&&(data[i]!='\n')&&(data[i]!='\'')&&(i<500));i++) ;
			if(strncmp(data+1,"phonetic",8)==0)
				for(i=9;((data[i]!=0)&&(data[i]!='\r')&&(data[i]!='\n')&&(data[i]!='\'')&&(i<500));i++) ;
			if(data[i]=='\'')
			{
				i=i+1;
				//convert to upper-case and find end of words data...
				for(j=i;((data[j]!=0)&&(data[j]!='\r')&&(data[j]!='\n')&&(data[j]!='\'')&&(j<500));j++)
				{
			...
			case 'E':
				switch(word[i+1])
				{
					...
					case 'E':
						...
						break;
					case 'D':
						if((i>0)&&(word[i+2]==0)&&(strlen(word)>3)&&((word[i-1]=='K')||(word[i-1]=='P')))
						{
							i++;
							printf("191"); //TT
						}
...
void next(char **statement)
{
	...
	else // step other than 1 or -1
	{
		...
		else if (strictatoi(forstep[numfors])&128) bcs(forlabel[numfors]);
		else
		{
			bcc(forlabel[numfors]);
...
void boxcollision(char **statement)
{
	...
	if (collisionwrap==0) 
	{
		...
		printf("  lda #(%s-1)\n",statement[10]);                        //boxw1
                printf("  sta temp4\n");
		printf("  lda "); printimmed(statement[12]); printf("%s\n",statement[12]); //boxx2
                printf("  sta temp5\n");
		printf("  lda "); printimmed(statement[14]); printf("%s\n",statement[14]); //boxy2
                printf("  sta temp6\n");
		printf("  lda #(%s-1)\n",statement[16]);                        //boxh2
                printf("  sta temp7\n");
		printf("  lda #(%s-1)\n",statement[18]);                        //boxw2
